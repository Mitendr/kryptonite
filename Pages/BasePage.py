...
class BaseClass:
    ...
    def get_ele_from_list(self, locator):
        ...
        elif str(locator).endswith("css"):
            elements = self.driver.find_elements(By.CSS_SELECTOR, confreader.read_config("Locators", locator))
    ...
    def selectFromDropdown(self, locator):
        if str(locator).endswith("xpath"):
            ele = self.driver.find_element(By.XPATH, confreader.read_config("Locators", locator))
        drp = Select(ele)
        return drp

    def scroll_for_ele(self, ele):
